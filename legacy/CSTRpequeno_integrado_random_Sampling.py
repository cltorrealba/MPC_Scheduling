...
if __name__ == "__main__":
    ...
    start = time.time()
    D_SDAsol,routeDSDA,_=solve_with_dsda(model_function=build_cstrs,model_args={'NT':5},starting_point=initialization,ext_dict=ext_ref,ext_logic=problem_logic_cstr, k='Infinity',provide_starting_initialization=True,feasible_model='dsda', subproblem_solver=nlp_solver, iter_timelimit=10,tee=False,global_tee=False)
    end = time.time()
    ...
    #SOLVE WITH MINLP
    model = build_cstrs_minlp_gdp(5,initialization)
    m_init=initialize_model(m=model,json_path=init_path)
    ...
    #SOLVE WITH GDP  
    model = build_cstrs_minlp_gdp(5,initialization) #TODO THIS CODE INITIALIZE YF AND YR ONLY, BUT NOT YP 
    m_init=initialize_model(m=model,json_path=init_path)
