...
Measures build + solve time for a small fermentation model (API v2) and optional scheduling model.
Outputs JSON report to stdout or a file (--output path).

Usage (PowerShell example):
  python -m biorefinery.scripts.benchmark_baseline --repeat 3 --include-scheduling --output results/benchmark_baseline.json
"""
...
def main(argv: List[str] | None = None):
    ...
    ap.add_argument("--repeat", type=int, default=1, help="Number of repetitions for each trial")
    ap.add_argument("--include-scheduling", action="store_true")
    ap.add_argument("--output", type=str, help="Path to write JSON report")
    ...
    report = {
        "fermentation": {
            "trials": ferm_trials,
            "summary": ferm_summary,
        },
        "scheduling": {
            "trials": sched_trials,
            "summary": sched_summary,
        } if args.include_scheduling else None,
        "repeat": args.repeat,
