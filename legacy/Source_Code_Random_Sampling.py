...
from feasibility_functions import feasibility_1,feasibility_2_modified
import copy
import time
import logging
...
def build_master(lower_b,upper_b,current):
...
if __name__ == "__main__":
    ...
    #         m_fix = external_ref(m=m_init,x=i,extra_logic_function=problem_logic_cstr,dict_extvar=reformulation_dict,tee=False)
    #         m_sol=solve_subproblem(m=m_fix, subproblem_solver=nlp_solver, timelimit=10000, tee=False)
            
    #         if m_sol.dsda_status=='Optimal':
    #             D_random[tuple(i)]=pe.value(m_sol.obj)
    #         else:
    #             D_random[tuple(i)]=infinity_val
    # end = time.time()
    # print('Random_sampling_time',end - start)
    # print('Random points generated:',D_random)


    ###IMPROVED RANDOM SAMPLING (WITH POTENTIAL PARALELLIZATION, algouth here i prefer to avoid the solution of the same problem twice)
    ...
    initial_Stage=0 #stage where the algorithm will be initialized: 1 is feasibility1, 2 is feasibility2 and 3 is optimality. Initialized at 0
    ...
    model = build_cstrs(15)
    init_path = generate_initialization(m=model) #user provided initialization (TODO: CHECK, BUT I THINK THIS IS THE USER PROVIDED)
    ...
    if random_points_number >= 1:
        ...
        print('Use more than one random_points_number')
    print(D_feasibility1)
    print(D_feasibility2)
    print(D_optimality)
    ...
    print('Random_sampling_time',end - start)
    print('Random points generated:',D_random,'Initialize algorithm at stage:',initial_Stage)
#until this point the initialization for the best solution obtained has been stored (if feasible, otherwise store user provided initialization)

    ######

   
    ####SOLUTION WITH DIFFERNT METHODS
    ...
    #     m=build_master(lower_bounds,upper_bounds,x_actual)
    #     #print(x_actual)
    ...
    # end = time.time()
    # print('method_1 time:',end - start,'method_1 obj:',D[tuple(x_actual)])
    ...
    #         x_actual=[round(pe.value(m.x1)),round(pe.value(m.x2))]
    # end = time.time()
    # print('method_2 time:',end - start,'method_2 obj:',D[tuple(x_actual)])
    ...
    # end = time.time()
    # print('method_3 time:',end - start,'method_3 obj:',D[tuple(x_actual)])
    ...
    #         x_actual=[round(pe.value(m.x1)),round(pe.value(m.x2))]
    # end = time.time()
    # print('method_4 time:',end - start,'method_4 obj:',D[tuple(x_actual)])
    ...
    # ### TODO CORREGIR DE A CUERDO CON LOS CODIGOS DONDE ESTA LO DE LA INICIALIZACION
    # ### SOLVE WITH DSDA
    # # model = build_cstrs(15)
    # # init_path = generate_initialization(m=model)
    ...
    # # start = time.time()
    # # D_SDAsol,routeDSDA,_=solve_with_dsda(model_function=build_cstrs,model_args={'NT':15},starting_point=initialization,ext_dict=ext_ref,ext_logic=problem_logic_cstr, k='Infinity',provide_starting_initialization=True,feasible_model='dsda', subproblem_solver=nlp_solver, iter_timelimit=100,tee=False,global_tee=False)
    # # end = time.time()
    ...
    # # #SOLVE WITH MINLP
    # # model = build_cstrs_minlp_gdp(15,initialization)
    # # m_init=initialize_model(m=model,json_path=init_path)  #TODO THIS CODE INITIALIZE YF AND YR ONLY, BUT NOT YP

    # # start = time.time()
    # # sub_opt={'add_options':['GAMS_MODEL.optfile = 1;','\n','$onecho > sbb.opt \n','rootsolver '+nlp_solver+'\n','subsolver '+nlp_solver+'\n','$offecho \n']}
    # # m_solved = solve_with_minlp(m_init, transformation='bigm', minlp='sbb', minlp_options=sub_opt,gams_output=False,rel_tol=0)
    # # end = time.time()
    ...
    # # start = time.time()
    # # m_solved = solve_with_gdpopt(m_init, mip='cplex',nlp=nlp_solver, strategy='LOA', mip_output=False, nlp_output=False,rel_tol=0,tee=False)
    # # end = time.time()
