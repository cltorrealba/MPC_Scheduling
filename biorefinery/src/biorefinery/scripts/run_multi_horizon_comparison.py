...
def main():
    ...
    ap.add_argument('--log-dir', default=None, help='Override directory for KPI CSV and JSON outputs (implies out-dir if provided).')
    args = ap.parse_args()

    if args.log_dir:
        base = pathlib.Path(args.log_dir)
        args.out_dir = str(base / 'json')
        args.csv = str(base / 'performance_log.csv')
