...
if __name__ == "__main__":
    ...
    model = build_cstrs(15)
    m_init_fixed = external_ref(m=model,x=initialization,extra_logic_function=problem_logic_cstr,dict_extvar=reformulation_dict,tee=False)
    ...
    if m_init_solved.dsda_status!='Optimal':
        ...
        for k in iterations:
            ...
            x_dict[k]=x_actual
            new_values,_=solve_subproblem_and_neighborhood_FEAS1(x_actual,neigh,D,infinity_val,Adjustable_val,reformulation_dict,problem_logic_cstr)
        ...
        #m_first_values = external_ref(m=model,x=x_actual,extra_logic_function=problem_logic_cstr,dict_extvar=reformulation_dict,tee=False)
        ...
        for k in iterations:
            ...
            #update current value of x in the dictionary
            x_dict[k]=x_actual
            new_values,_,path_result=solve_subproblem_and_neighborhood_FEAS2(x_actual,neigh,D,infinity_val,Adjustable_val,reformulation_dict,problem_logic_cstr,nlp_solver,init_path)
            fobj_actual=list(new_values.values())[0]
            fval_dict[k]=fobj_actual  
            #Add points to D
            D.update(new_values)
    ...
    if m_init_solved.dsda_status!='Optimal':
        ...
        for k in iterations:
            ...
            #update current value of x in the dictionary
            x_dict[k]=x_actual
            new_values,feasible_n=solve_subproblem_and_neighborhood_FEAS1(x_actual,neigh,D,infinity_val,Adjustable_val,reformulation_dict,problem_logic_cstr)
            #print(new_values)
            #print(feasible_n)
            #print(new_values)
            #print(feasible_n)
        ...
        for k in iterations:
            ...
            #update current value of x in the dictionary
            x_dict[k]=x_actual
            new_values,feasible_n,path_result=solve_subproblem_and_neighborhood_FEAS2(x_actual,neigh,D,infinity_val,Adjustable_val,reformulation_dict,problem_logic_cstr,nlp_solver,init_path)
            #print(new_values)
            #print(feasible_n)
            #print(new_values)
            #print(feasible_n)
...
#    m_first_values = external_ref(m=model,x=initialization,extra_logic_function=problem_logic_cstr,dict_extvar=reformulation_dict,tee=False)
    ...
    #m_fixed = external_ref(m=m_initialized,x=x_actual,extra_logic_function=problem_logic_cstr,dict_extvar=reformulation_dict,tee=False)
    ...
    try:  
        start = time.time()
        D_SDAsol,routeDSDA,_=solve_with_dsda(model_function=build_cstrs,model_args={'NT':15},starting_point=initialization,ext_dict=ext_ref,ext_logic=problem_logic_cstr, k='Infinity',provide_starting_initialization=True,feasible_model='dsda', subproblem_solver=nlp_solver, iter_timelimit=1000,tee=False,global_tee=False)
        end = time.time()
        ...
        mDSDA_fixed = external_ref(m=D_SDAsol,x=routeDSDA[-1],extra_logic_function=problem_logic_cstr,dict_extvar=reformulation_dict,tee=False)
    ...
    init_path = generate_initialization(m=m_init_solved)
    #NOW SOLVE  
    model = build_cstrs_minlp_gdp(15,initialization) #TODO THIS CODE INITIALIZE YF AND YR ONLY, BUT NOT YP 
    m_init=initialize_model(m=model,json_path=init_path)
    try:
        start = time.time()
